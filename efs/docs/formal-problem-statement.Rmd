---
title: "A predictive model to better evaluate the student loan default risks"
author: "Sanatan Das"
date: "April 08, 2018"
output:
  pdf_document:
...
This study utilizes new data, released by the U.S. Department of Education in October 2017, linking two waves of the Beginning Postsecondary Student (BPS) survey, a nationally-representative survey of first-time college beginners, to administrative data on debt and defaults. This allows for the most comprehensive assessment yet of student debt and default from the moment students first enter college, to when they are repaying loans up to 20 years later, for two cohorts of first-time entrants (1995-96 and 2003-04 entrants, referred as BPS-96 and BPS-04).
...
If we assume that the cumulative defaults grow at the same rate (in percentage terms) for the 2004 cohort as for the earlier cohort, we can project how defaults are likely to increase beyond year 12 for the 2004 cohort. To compute these projections, we first use the 1996 cohort to calculate the cumulative default rates in years 13-20 as a percentage of year 12 cumulative default rates. We then take this percentage for years 13-20 and apply it to the 12-year rate observed for the 2004 cohort. So, for example, since the 20-year rate was 41 percent higher than the 12-year rate for the 1996 cohort, We project the Year 20 cumulative default rate for the 2004 cohort is projected to be 41 percent higher than its 12-year rate.
...
**T**he analysis presented above highlight the value of tracking individual students from the beginning of their college trajectory for many years beyond when they leave school, and the importance of disaggregating trends by student and institutional characteristics.  Key findings include:
...
**Now, it is the responsibility of the Banks and Credit Unions to do a thorough analysis on the loan application and the student data before approving their loan.**
...
# The Solution: Education Loan Assistant&trade;
...
**T**he Banks and Credit Unions spends billions of dollars every year for recovering the student loan debt. Feferal Student Aid releases multiple datasets and reports every year for the Official Cohort Default Rates for Schools. These datsets are available in their website for download in different formats (excel, csv, html and pdf files). They also publish the detailed instructions how to use these data files. They publish the field definitions, abbreviations used etc. so that the values can be extracted from those information. To solve this problem of the lending organizations, we analyze these data and create a model to help them to reduce the number of defaults.

### Data Collection

**T**he first step for this project is to collect those individual files and go through them, looking for the important information. Then we need to put them together into a single format (csv). The data cleaning, verifying the value types (char, numeric etc) need to be completed by looking at the data (before we start the thorough cleaning and making it tidy using R(it's a programming language)).

### Exploratory Data Analysis and Visualization

**O**nce the dataset is ready, we need to start on the Exploratory Data Analysis, creating visual representations, plotting relationship graphs, finding strong and weak features, finding outliers etc. The whole exercise will produce a set of features (a broader set) that will be used for creating models.
...
**T**he goal of this solution is to help the Student Loan organizations to make well informed decisions while approving any educational loan application. Currently, they are already having a high rate of the default applications as mentioned in the report. It can't get worse than as it is today. This project and the model comes with some low severity risks with it. We can list down the risk statements clearly as below so that the business is aware of it before delving into the model development.

* The proposed model is built on the data released by the U.S. Department of Education. It is assumed that those data are correct and provide the right trend of the student loan debt and default. The machine learning model is all based on the data we trust. If there is any issue on the provided data, it will impact the model and prediction results.
...
**T**he tool 'Education Loan Assistant&trade;' analyzes the data based on student debt and repayment information, released by the U.S. Department of Education and creates a predictive model. To achieve more accuracy on the model prediction, we can use the private and confidential data of the Bank and Credit Unions (if we create a model specific to the organization and every lending organization have their customer database). Those data are more realistic and accurate to use and will provide better prediction results.
